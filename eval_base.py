...
def _parse_args():
	...
	parser.add_argument('--load_model_path', type=str, default='model.pt', help='path to load model from')
...
class Evaler():
	def __init__(self, args):
		...
		self.model = Transformer(device=self.device, d_model=self.d_model, nhead=4, dim_feedforward=2048, num_layers=3, window_size=5, pred_cutoff=0.2).to(self.device)
	...
	def eval(self, data):
		...
		log_probs = torch.transpose(self.model.forward(x).cpu(), 0, 1)
